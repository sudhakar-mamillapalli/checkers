...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: sudhakarmamillapalli.checkers.checkers initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.SudhakarmamillapalliCheckersCheckers.query.queryParams()).data
		...
		async sendMsgCreateGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.SudhakarmamillapalliCheckersCheckers.tx.sendMsgCreateGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgRejectGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.SudhakarmamillapalliCheckersCheckers.tx.sendMsgRejectGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgPlayMove({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.SudhakarmamillapalliCheckersCheckers.tx.sendMsgPlayMove({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgCreateGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SudhakarmamillapalliCheckersCheckers.tx.msgCreateGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateGame:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgRejectGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SudhakarmamillapalliCheckersCheckers.tx.msgRejectGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRejectGame:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgPlayMove({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SudhakarmamillapalliCheckersCheckers.tx.msgPlayMove({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPlayMove:Init Could not initialize signing client. Wallet is required.')
